...
def generate_token():
    ...
    user = query("SELECT lastwrongpassword,wrongpasswordcount,hash,userid FROM users WHERE email =%(email)s", {'email': email})
    if len(user) == 0:
    ...
    if int(user['wrongpasswordcount']) >= ATTEMPTS_BEFORE_COOLDOWN:
        if int((datetime.datetime.now() - user['lastwrongpassword']).total_seconds()) > COOLDOWN_TIME_SECONDS:
            query_update("UPDATE users SET wrongpasswordcount = 0 where userid = %(userid)s", {'userid': user['userid']})
        else:
            return make_response("Acces Denied, your account is blocked for " +
                                 str(int((datetime.datetime.now() - user['lastwrongpassword']).total_seconds())-ATTEMPTS_BEFORE_COOLDOWN) +
                                 " more seconds", 401)
