...
def move_file(project_id):
    ...
    backup_path_full = fs_service.pathify(
        config.FILE_STORAGE_ROOT, fs_service.get_project_path(project_id, "backup"), source_path)
    ...
    backup_target_folder = fs_service.pathify(
        config.FILE_STORAGE_ROOT, fs_service.get_project_path(project_id, "backup"), target_path)
    ...
    try:
        ...
        try:
            ...
            shutil.move(fs_service.pathify(backup_target_folder, file_name), backup_source_folder)
...
def rename_file(project_id):
    ...
    backup_file_path = fs_service.pathify(
    ...
    if fs_service.file_exists(backup_file_path):
        if not fs_service.file_rename_valid(backup_file_path, new_name):
    ...
    if not fs_service.file_rename_valid(file_path, new_name):
        ...
        directory, old_name = backup_file_path.rsplit('/', 1)[1]
...
def read_deleted(project_id):
    ...
    project_backup_root = fs_service.pathify(config.FILE_STORAGE_ROOT, fs_service.get_project_path(project_id, "backup"))
...
def restore_file(project_id):
    ...
    backup_path = fs_service.pathify(config.FILE_STORAGE_ROOT, fs_service.get_project_path(project_id, "backup"), path)
